...
- **admin 工作台**:`/admin/reports` 三 tab(PENDING/RESOLVED/ALL),`AdminGuard` 走 `User.role === "ADMIN"` fail-closed(老 token / 无 role 字段一律拒绝),非 admin → 403 `ADMIN_REQUIRED`。`ResolveDialog` 选 OFFLINE/WARN/DISMISS + note(≤200 字),OFFLINE 选中时显示红字高危确认。OFFLINE 处置走 `prisma.$transaction` 同时更新 `Report.status=RESOLVED` + `Draft.status=OFFLINE` + `offlineReason`。

...
### 2.2 为什么删 politics/drugs/medical(仅指 SENSITIVE_CATEGORIES 模型评测)

> **范围澄清**:`SAFETY_KEYS`(preflight 6 维质量评分,如 pornography/gambling/drugs/politics/vulgarity/false_advertising)与 `SENSITIVE_CATEGORIES`(prompt/段落/发布后命中分类)是**两套独立体系**,本期只动后者。preflight 6 维分维度保持不变,LLM 仍会给 drugs/politics/vulgarity 打分作为作者提醒,只是不再做命中级模型评测。
...
| `apps/api/src/reviews/review.service.ts`                                          | `parseSafetyOf7Cats` 重命名为 `parseSafetyByCategories`(函数体已是 `SENSITIVE_CATEGORIES.map`,enum 改了自动 6 项),SAFETY_REVIEW prompt body 同步 6 类目说明文案;`parseSafetyOf6Cats`(preflight 用的 6 维质量评分,走 `SAFETY_KEYS`)**与本期无关,不动** |
...
| `apps/web/src/app/drafts/[id]/_components/`(若存在 SENSITIVE_CATEGORIES 文案映射) | 检查并同步 6 维(`ScorePanel.tsx` 用的是 SAFETY_KEYS preflight 维度,**与本期无关不动**)                                                                                                                                                                |

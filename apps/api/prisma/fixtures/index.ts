 ...
 *
 * 删除顺序固定为:Review → DraftVersion → Draft → Prompt → User
 *   FK 依赖:Review.draftId → Draft、DraftVersion.draftId → Draft、Draft.authorId → User、
 *   Prompt.authorId → User、Prompt.sourcePromptId → Prompt(自引用,SET NULL)
 *
 * 写入顺序反过来:User → Prompt → Draft → Review(回写 Draft.lastReviewId)
 */
